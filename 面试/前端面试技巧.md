...
var s6 = new Child4()
console.log(s5, s6)
// 无法区分对象s5是由他的子类实例化Child4还是父类实例化Parent4
console.log(s5 instanceof Child4, s5 instanceof parent4) // true true
console.log(s5.constructor) // Parent4 (其实在组合方式继承中也有这个问题)
```
...
Child5.prototype = Object.create(Parent5.prototype)// 通过中间对象来区分父类和子类的构造函数
Child5.prototype.constructor = Child5 // 然后重新定义Child5原型对象的constructor
// 不能直接在优化一中重新定义子类的constructor,是因为优化一中Parent4和Child4的原型对象引用的是同一个对象，重新定义相当于同时修改了父类的constructor
var s7 = new Child5()
...
>
> 同时也不要直接写出最完美的实现方式，条件允许的情况下，尽量将所有的继承方式都写出来并说出它们的优缺点，在展现自己的知识的同时可以在一定的时间里减少面试题目的数量。
