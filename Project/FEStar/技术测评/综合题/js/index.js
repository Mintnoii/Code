...
var bestResultContainer = $('#bestResult');
var gameOverBoard = $("#gameOverBoard");
...
function initGame(){
	...
	gameScore = 0;
	gameOverBoard.css('display','none');
	scoreContainer.text(0);
	bestResultContainer.text(bestResult);
	gameLayer.remove();
	numberLayer.remove();
    createRandom();
    drawBack();
...
function refresh(){
	...
	if(result>0){
		...
		scoreContainer.append(addition);
		total = [0];
	}
	gameLayer.remove()
	numberLayer.remove()
